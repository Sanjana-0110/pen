    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '.add-to-cart', function() {
            ...
            if (pincode !== null && pincode !== "") {
                ...
                // Send cart data and pin code to the server
                $.ajax({
                    type: 'POST',
                    url: 'PinCodeVerificationServlet',
                    contentType: 'application/json; charset=utf-8',
                    data: JSON.stringify({ cart: [productData], pincode: pincode }), // Send productData as an array
                    success: function(response) {
                        // Handle success response
                        if (response.productAvailable) {
                            // Product is available, proceed to add to the cart
                            var cartItems = JSON.parse(localStorage.getItem('cart') || '[]');
                            cartItems.push(productData);
                            localStorage.setItem('cart', JSON.stringify(cartItems));
                            alert('Product added to cart and cart data saved to session!');
                        } else {
                            // Product is not available, display an alert
                            alert('Product cannot be added to cart. It is not available.');
                        }
                    },
                    error: function(xhr, status, error) {
                        // Handle error response
                        alert('Product cannot be delivered!');
